defmodule Queries do
    ...
    def insertMap(map, prikey, type) do
      {:ok, conn} = Helperfunction.connection()
      statement = "UPDATE probuildstest.indivisualstats SET #{type} = :items WHERE id = :prikey"
      {:ok, %Xandra.Void{}} = Xandra.execute(conn, statement, %{
    ...
    def insertPlayerHistory(response) do
        ...
        for x <- 0..2 do
          ...
          {:ok, %Xandra.Void{}} = Xandra.execute(conn, statement, %{
            ...
            # alrunes: {"list<int>", [Enum.at(partStats, x) |> Map.get("perk0"),Enum.at(partStats, x) |> Map.get("perk1"),Enum.at(partStats, x) |> Map.get("perk2"),Enum.at(partStats, x) |> Map.get("perk3"),Enum.at(partStats, x) |> Map.get("perk4"),Enum.at(partStats, x) |> Map.get("perk0"),Enum.at(partStats, x) |> Map.get("perk0"),]}
          })
